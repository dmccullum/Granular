...
fail() {
    echo
    echo "Filmify could not be built."
    echo "$1"
    pause_before_exit
    exit 1
}

cd "$ROOT" || fail "The Filmify source folder could not be opened."

MACOS_MAJOR="$(/usr/bin/sw_vers -productVersion | /usr/bin/cut -d. -f1)"
if (( MACOS_MAJOR < 26 )); then
    fail "Filmify requires macOS 26 or later."
fi
...
if [[ ! -x "$ICON_TOOL" ]]; then
    fail "Filmify needs the full Xcode 26 application, including Icon Composer. Command Line Tools alone are not enough."
fi

echo "Building Filmify…"
echo
...
if [[ ! -d "$APP" ]]; then
    fail "The build finished without creating dist/Filmify.app."
fi

echo
echo "Filmify is ready:"
echo "$APP"
echo
echo "Drag Filmify.app into Applications if you want to install it."

if [[ "${FILMIFY_SKIP_REVEAL:-0}" != "1" ]]; then
    /usr/bin/open -R "$APP"
    pause_before_exit
fi
